...
func StartNew() {
	...
	r.HandleFunc("/users/settings", user.Update).Methods("PUT")
	r.HandleFunc("/users/me", user.SelfProfile).Methods("GET")
	r.HandleFunc("/users/playlists", playlist.GetUserPlaylists).Methods("GET")
	r.HandleFunc("/users/albums", album.GetUserAlbums).Methods("GET")
	r.HandleFunc("/playlists/{id:[0-9]+}", playlist.GetFullPlaylistById).Methods("GET")
	r.HandleFunc("/albums/{id:[0-9]+}", album.GetFullAlbum).Methods("GET")
	r.HandleFunc("/users/profiles/{profile}", user.Profile)
	r.HandleFunc("/users/images", user.UpdateAvatar).Methods("POST")
	r.HandleFunc("/users", user.CheckAuth)
	r.HandleFunc("/users/signup", user.Create).Methods("POST")
	r.HandleFunc("/users/login", user.Login).Methods("POST")
	r.HandleFunc("/users/logout", user.Logout).Methods("DELETE")
